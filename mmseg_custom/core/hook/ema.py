...
class BaseEMAHook(Hook):
    ...
    Args:
        ...
        auto_resume (bool, optional): Auto resume from latest checkpoint.
            Defaults to True.
    ...
    def before_run(self, runner):
        ...
        if self.checkpoint is not None:
            runner.logger.info(f'Resume checkpoint in EMA Hook...')
    ...
    def after_train_iter(self, runner):
        ...
        if (runner.iter + 1) % self.eval_interval == 0:
            runner.logger.info(f'Swap parameters (after train) after iter [{runner.iter + 1}]')
    ...
    def before_train_iter(self, runner):
        if (runner.iter) % self.eval_interval == 0:
            runner.logger.info(f'Swap parameters (before train) before iter [{runner.iter + 1}]')
            self._swap_ema_parameters()
...
@HOOKS.register_module()
class ConstantMomentumEMAHook(BaseEMAHook):
    """EMAHook using constant momentum strategy.

    """

    def __init__(self, **kwargs):
        super(ConstantMomentumEMAHook, self).__init__(**kwargs)
        self.momentum_fun = None
